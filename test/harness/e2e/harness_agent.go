...
func (h *Harness) UpdateDeviceSpecWithEvidence(deviceID, artifactDir, filename, description string, updateFunc func(*v1beta1.Device)) error {
	...
	if err != nil {
		_ = WriteEvidence(artifactDir, filename, description, "prepare next rendered version failed", err)
		return err
	}
	if err := h.UpdateDeviceWithRetries(deviceID, updateFunc); err != nil {
		_ = WriteEvidence(artifactDir, filename, description, "update device failed", err)
		return err
	}
	if err := h.WaitForDeviceNewRenderedVersion(deviceID, expectedRenderedVersion); err != nil {
		_ = WriteEvidence(artifactDir, filename, description, "wait for rendered version failed", err)
		return err
	}
	output := fmt.Sprintf("updated to renderedVersion=%d", expectedRenderedVersion)
	if writeErr := WriteEvidence(artifactDir, filename, description, output, nil); writeErr != nil {
		return writeErr
	}
...
func (h *Harness) PrepareVMAccess(deviceID, artifactDir string) error {
	...
	if err != nil {
		_ = WriteEvidence(artifactDir, "host_console_whoami.txt", "flightctl console device/"+deviceID+" -- whoami", consoleOut, err)
		return err
	}
	if err := WriteEvidence(artifactDir, "host_console_whoami.txt", "flightctl console device/"+deviceID+" -- whoami", consoleOut, nil); err != nil {
		return err
	}
	...
	sshWhoamiOutput := ""
	if sshWhoamiOut != nil {
		sshWhoamiOutput = sshWhoamiOut.String()
	}
	if err != nil {
		_ = WriteEvidence(artifactDir, "vm_ssh_whoami.txt", "whoami", sshWhoamiOutput, err)
		return err
	}
	if err := WriteEvidence(artifactDir, "vm_ssh_whoami.txt", "whoami", sshWhoamiOutput, nil); err != nil {
		return err
	}
