...
var _ = Describe("Agent image and artifact pruning", func() {
	...
	Context("image and artifact pruning", func() {
		It("Enable/Disable Pruning via Agent Config and Drop-ins", Label("87084", "sanity", "agent"), func() {
			...
			Expect(err).ToNot(HaveOccurred())
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_v125_disabled_pruning.txt", "quay.io/library/nginx.25")
			...
			Expect(err).ToNot(HaveOccurred())
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest_disabled_pruning.txt", "quay.io/library/nginx\"")
		...
		It("References File Created and Updated Before Upgrades", Label("87085", "sanity", "agent"), func() {
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_inline.txt", "quay.io/library/nginx\"")
			nginxInlineSpec, err := buildInlineAppSpec("nginx-server", v1beta1.AppTypeQuadlet, []inlineContent{
		...
		It("Reference Entry Removed Only After Actual Image Is Removed", Label("87086", "sanity", "agent"), func() {
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_v125.txt", "quay.io/library/nginx.25")
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest.txt", "quay.io/library/nginx\"")
		...
		It("Lost Reference Model: Unreferenced Images Become Eligible and Are Pruned", Label("87087", "sanity", "agent"), func() {
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_postgres_base.txt", "quay.io/library/postgres:16")
			postgresBaseSpec, err := buildInlineAppSpec("postgres-db", v1beta1.AppTypeQuadlet, []inlineContent{
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_postgres_alt.txt", fmt.Sprintf("quay.io/library/postgres:%s", altTag))
			postgresAltSpec, err := buildInlineAppSpec("postgres-db", v1beta1.AppTypeQuadlet, []inlineContent{
				{Path: "db.volume", Content: postgresVolumeContent},
		...
		It("Application Image Retention: Keep Current + Previous, Prune Older", Label("87088", "sanity", "agent"), func() {
			...
			By("apply nginx.25")
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_v125_retention.txt", "quay.io/library/nginx.25")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_v125.txt", "update device applications: nginx v1.25", nginxV125Spec)
			Expect(err).ToNot(HaveOccurred())

			By("apply nginx")
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest_retention.txt", "quay.io/library/nginx\"")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_latest.txt", "update device applications: nginx latest", nginxLatestSpec)
			Expect(err).ToNot(HaveOccurred())
		...
		It("OCI Artifacts: Unreferenced Artifacts Are Pruned", Label("87089", "sanity", "agent"), func() {
			...
			artifactSpec, err := buildImageAppSpec("my-compose-app", v1beta1.AppTypeCompose, "quay.io/my-org/my-compose-app.0")
			Expect(err).ToNot(HaveOccurred())
		...
		It("Pruning Runs Only After Successful Reconciliation and Is Non-Blocking", Label("87091", "sanity", "agent"), func() {
			...
			brokenSpec, err := buildInlineAppSpec("broken", v1beta1.AppTypeQuadlet, []inlineContent{
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest_non_blocking.txt", "quay.io/library/nginx\"")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_latest.txt", "update device applications: nginx latest", nginxLatestSpec)
			Expect(err).ToNot(HaveOccurred())
		...
		It("References File Updated After Successful Pruning", Label("87092", "sanity", "agent"), func() {
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_v125_refs_update.txt", "quay.io/library/nginx.25")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_v125.txt", "update device applications: nginx v1.25", nginxV125Spec)
			Expect(err).ToNot(HaveOccurred())

			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest_refs_update.txt", "quay.io/library/nginx\"")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_latest.txt", "update device applications: nginx latest", nginxLatestSpec)
			Expect(err).ToNot(HaveOccurred())
...
podman ps | grep hold-nginx`
			...
			assertReferenceAbsent(harness, artifactDir, "vm_refs_cat_before_nginx_latest_partial_failure.txt", "quay.io/library/nginx\"")
			err = harness.UpdateApplicationsWithEvidence(deviceID, artifactDir, "host_device_update_nginx_latest.txt", "update device applications: nginx latest", nginxLatestSpec)
			Expect(err).ToNot(HaveOccurred())
...
func assertReferenceAbsent(harness *e2e.Harness, artifactDir, filename, reference string) {
	...
	Expect(err).ToNot(HaveOccurred())
	Expect(refsOut).ToNot(ContainSubstring(reference))
...
func buildNginxSpec(containerContent string) (v1beta1.ApplicationProviderSpec, error) {
	...
	return buildInlineAppSpec("nginx", v1beta1.AppTypeQuadlet, []inlineContent{
...
func buildInlineAppSpec(appName string, appType v1beta1.AppType, contents []inlineContent) (v1beta1.ApplicationProviderSpec, error) {
	GinkgoWriter.Printf("Building inline app spec (name=%s type=%v contents=%d)\n", appName, appType, len(contents))
	...
	case v1beta1.AppTypeQuadlet:
		...
		return appSpec, nil
	default:
		return v1beta1.ApplicationProviderSpec{}, fmt.Errorf("inline application provider spec not supported for app type %s", appType)
	}
}

func buildImageAppSpec(appName string, appType v1beta1.AppType, image string) (v1beta1.ApplicationProviderSpec, error) {
	GinkgoWriter.Printf("Building image app spec (name=%s type=%v image=%s)\n", appName, appType, image)
	var appSpec v1beta1.ApplicationProviderSpec
	switch appType {
	case v1beta1.AppTypeCompose:
